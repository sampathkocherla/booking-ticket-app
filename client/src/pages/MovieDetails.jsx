...
const Moviedetails = () => {
  ...
  useEffect(() => {
    // Ensure Heart is in correct state
    setIsFavorite(favorites?.some(favId => favId.toString() === id.toString()) || false);
  }, [favorites, id]);
  ...
  const handleFavorite = async () => {
    ...
    try {
      ...
      if (data.success) {
        await  fetchFavorites(); // Refresh favorites from backend
        toast.success(data.message);
      } else {
        setIsFavorite(prev => !prev); // rollback UI if failed
        toast.error(data.message);
      }
    } catch (error) {
      ...
      setIsFavorite(prev => !prev); // rollback UI
    ...
    <div className="px-4 md:px-8 lg:px-16 xl:px-20 py-12 mt-28 max-md:mt-15 overflow-hidden">
      <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
        ...
        <div className="text-white relative flex flex-col gap-4 max-w-xl">
          ...
          <div className="flex flex-wrap gap-4 mt-4 max-md:gap-2">
            ...
            >
              <Heart
                className={`w-5 h-5 ${isFavorite ? 'text-primary fill-primary' : ''}`}
              />
            </button>
          </div>
        </div>
      ...
      <div className="px-6 md:px-8 lg:px-16 xl:px-20 overflow-hidden py-10 max-md:py-0">
        <div className="relative flex items-center justify-between pt-20 pb-5 pl-10 text-lg max-md:pl-0">
          ...
          <p className="text-gray-300 font-medium max-md:text-sm text-lg">
            You May Also Like
          </p>
          <button
